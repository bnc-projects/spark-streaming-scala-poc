...
import com.analytics.spark.kinesis.{KinesisClientConfig => KCL}
...
case class Record(id: String, hostname: String, legacyid: BigInt, name: String, symbol: String, status: String, totalSupplyUrl: String, totalSupply: BigInt, freeFloatSupply: BigInt, timestamp: String, assetId: String)
...
object StreamingAnalyzer {
  ...
  def execute(config: StreamingConfig) {
    ...
    val kinesisClient = KCL.setupKinesisClientConnection(config.endpointUrl, config.awsProfile)
    ...
    val numShards = KCL.getShardCount(kinesisClient, config.streamName)
    ...
    items.foreachRDD((rdd: RDD[String], time: Time) => {
      ...
      if (rdd.count() > 0) {
        try {
          ...
          val supply = spark.sql("SELECT assetId, legacyid, name, symbol, freeFloatSupply, timestamp FROM records WHERE freeFloatSupply >= 31200000")
          supply.createOrReplaceTempView("supply")
          supply.show()

          val outPartitionFolder = outDateFormat.format(new Date(time.milliseconds))

          supply.rdd.coalesce(1).saveAsTextFile("%s/%s".format(s"hdfs:///${config.hdfsOutputLoc}", outPartitionFolder),
            classOf[GzipCodec])
